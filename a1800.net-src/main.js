...
function actionSend(action) {
  ...
  .then(response => {
    if (response.action =="contact")
      getID("response").innerHTML = response.text;
...
//        getID(response.target).outerHTML = response.content;  
        toggleEmtyGroups();  
        if (getID("template-list")) // after empty status change
          {
      ...
      if (response.target=="preview")
        {
          ...
          for (el of text)
            {
            if (el.style.display != "none" ) el.style.display = "none"; else el.style.display = "block";
            }
        }
...
function checkGUID(guid) {
  ...
  if (assetTags.hasOwnProperty(guid))
    return assetTags[guid]
  else 
    return null;
...
function createTabs (response) {
  ...
  if (response.data) 
    for (const key in response.data)
      assetTags[key] = response.data[key];

  if (response.xml) source = JSON.parse(response.xml); 

  if (getID("img-start")) getID("img-start").style.display = "none";

  if (response.target && response.target!="dataset")
    getID(response.target.replace("asset-","") + "Tab").click()
  else
    getID("dataTab").click();
  ...
  for (let array of assetValues) 
    {
      ...
      if (array[0] == "Values/Standard/IconFilename")
        getID("tab-title").insertAdjacentHTML("afterbegin", "<span class=\"item-picture-span\"><img class=\"item-picture\" src=\"" + DATA_URL + array[1].replace(".png","_0.png") + "\" onError=\"replaceImg(this)\"></span>");
  ...
  {  
    if (actions.length > 1)
      getID("back").innerHTML = "<button onClick=\"actions.pop();actionSend('" + actions[actions.length-2] + "')\" class=\"button-small\"><i class=\"fa fa-chevron-left\"></i> back</button>";
  }
  ...
  if (values)
    getID("asset-data").insertAdjacentHTML("beforeend", "<h3>Values: <div class=\"filter-system right\"><label for=\"toggleTips\"><input type=\"checkbox\" name=\"toggleTips\" value=\"1\" id=\"toggleTips\" onClick=\"toggleTips(this,'tips');\"> toggle hints</label></div></h3><div class=\"data\">" + values + "</div>");
   
  // xml snippet tab
  if (response.xml && source)
  {
    getID("asset-xml").innerHTML = response.path.replace("</i></div>"," / XML source</i></div>") + "<div class=\"txt-right\"><button onClick=copy2C() class=\"button-small\"><i class=\"fa fa-fw fa-file-text-o\"></i> Copy</button></div>";
    getID("asset-xml").insertAdjacentHTML("beforeend",createSnippet(source,response.action));
  }
  else 
    getID("asset-xml").innerHTML = "no XML data found...";
  
   // texts tab 
  if (response.txt)
    getID("asset-txt").innerHTML = response.path.replace("</i></div>"," / Texts</i></div>") + response.txt;
  else
    getID("asset-txt").innerHTML = "no Texts data found...";
  
  // img tab  

  log (Object.keys(response.files));

  if (Object.keys(response.files).length) // "array here must be"
    getID("asset-img").innerHTML = response.path.replace("</i></div>"," / Images</i></div>") + "<h3>Graphics files:</h3><p><br></p>" + loadIMG(response.files);
  else
    getID("asset-img").innerHTML = "no IMG data found...";
  ...
  if (response.action=="dataset" && response.data)
    ...
    if (queryS(".data-datasets",getID("up")))
       queryS(".data-datasets",getID("up")).remove();
...
function dataPath(source) {
  ...
  for (let data of source) 
  {
      ...
      if (data.AssetPath.includes("Template")) 
        ...
        if (data.AssetValue != goto.replace("asset-","")) 
        {
          if (title = checkGUID(data.AssetValue))
            value[1] = ("<span class=\"tooltip\"><a data-guid=\"" + data.AssetValue + "\" class=\"data-asset\" data-goto=\"" + goto + "\">" + data.AssetValue + "</a><a onClick=\"window.open(location.pathname + '?itemSearch=" + data.AssetValue +"', '_blank');\" class=\"link-ext\"><i class=\"fa fa-external-link-square\"></i></a><span class=\"tooltiptext\">" + title + "</span></span>"); // removes array numbers like /0/
        }
  ...
  for (let key in source) {
    ...
    else  
    {
      ...
      if (key=="Template" || key=="TemplateName") 
        ...
        if (source[key] != goto.replace("asset-","")) {
          if (title = checkGUID(source[key]))
            value[1] = ("<span class=\"tooltip\"><a data-guid=\"" + source[key] + "\" class=\"data-asset\" data-goto=\"" + goto + "\">" + source[key] + "</a><span class=\"tooltiptext\">" + title + "</span></span>");
        }
  ...
  for (let glb of files) 
    {
      ...
      {
      images += "<model-viewer class=\"viewer\" src=\"" + DATA_URL + glb + "\" auto-rotate rotation-per-second=\"200%\" camera-controls touch-action=\"pan-y\" shadow-intensity=\"0.25\" camera-target=\"0m 1m 0m\" interaction-prompt=\"none\" exposure=\"0.70\"><div slot=\"progress-bar\"></div></model-viewer>";
      ...
      {
        if (glb.indexOf("_diff_") !== -1) images += "<img class=\"img\" style=\"max-width:460px\" src=\"" + DATA_URL + glb + "\" onError=\"this.style.display='none'\"><p><i>diffusion file:</i><br><a href='" + glb + "' target=_blank>" + glb + "</a></p>";
        if (glb.indexOf("_norm_") !== -1) images += "<img class=\"img\" style=\"max-width:460px\" src=\"" + DATA_URL + glb + "\" onError=\"this.style.display='none'\"><p><i>normal map file:</i><br><a href='" + glb + "' target=_blank>" + glb + "</a></p>";
      }
    }  
...
//        images += "<p><h3>models for CFG file:</h3><a href='" + pic + "' target=_blank>" + pic + "</a><br></p>";
        ...
        for (file in files[pic])
          //log (files[pic][file]);
          images+= showGlb(files[pic][file],file);
      }
      else
      {
      ...
      images += "<div><p><img class=\"img\" src=\"" + DATA_URL + pic + "\" onError=\"this.style.display='none'\"><p><i>filename:</i><br><b>" + pic + "</b></p></div>";
...
function copy2C(copyText = '') {
  ...
  if (!copyText)
    copyText = getID("snippet").textContent;
   ...
   //log (el + " " + typeof obj[el]);
    if (obj[el] instanceof Array) 
    {
      ...
      else 
      {
        ...
        if (title)
          xml += indent + '<span class="tag-name">&lt;!--' + title + '--&gt;</span>\n';
...
//	let dataCSV = "\xEF\xBB\xBF";
  ...
  let fname = "";
    ...
    {  
      ...
      if (queryS('.template-name',item))
        dataCSV += ';"' + queryS('.template-name',item).innerText + '"'; else dataCSV += ';';
      if (queryS('.ext-info',item))
        dataCSV += ';"' + queryS('.ext-info',item).innerText.replace(": ","\";\"") + '"';
      if (queryS('.text',item))
        dataCSV += ';"' + queryS('.text',item).innerText + '"';
      dataCSV += "\n";
    }
  ...
  if (getID("tab-title") && getID("tab-title").innerText)
    fname = queryS('b',getID("tab-title")).innerText.replace("Template: ", "");
  else 
    fname = "from-search";
        ...
        if (row.className.indexOf("header") == -1 || !row.className.indexOf("header"))
          {
            ...
            if (filter.indexOf("-") === 0)
              {
                if (row.innerText.toUpperCase().indexOf(filter.replace("-","")) > -1 && filter.length > 1)
...
function showMore() {
  ...
  for (let line of querySA(".hidden",getID("asset-list")))
...
*/
    ...
    if (search = getID("search-results"))
      return search.getAttribute("data-search");
	...
	for (i = 0; i < data.length; i++) 
		...
		if (data.elements[i].type == 'checkbox' && data.elements[i].value == 1)
			...
			if (data.elements[i].checked) 
				query += data.elements[i].name + "=1&";	
			else
				query += data.elements[i].name + "=0&";	
			}
		else
			query += data.elements[i].name + "=" + data.elements[i].value + "&";	
		}
...
function showMessage() {
  ...
  show.className = "show";
  setTimeout(function(){ show.className = show.className.replace("show", ""); }, 1200);
}
...
function tip(tip = null) {
  ...
  if (tip) 
		content = getID('tip-'+tip); // tip name if more than one on site
	else	
		content = getID('tip');
...
function resNavi() {
 ...
 function showModal(content="", title="") {
	...
	if (content)
		getID("modal-body").innerHTML = content;
	if (title)
		getID("modal-title").innerHTML = title;
...
getID("container").addEventListener("click", e => {
...
if (e.target.parentElement.className=="show-modal")
{
  ...
  if (e.target.parentElement.getAttribute("data-guid"))
    showModal(e.target.parentElement.getAttribute("data-guid"));
...

{
  if (e.target.getAttribute("title"))
    copy2C(e.target.getAttribute("title"));
}
...

{
  ...
  if (classList.value.indexOf("data-") == 0 || (classList.contains("mark") && getID("search-results").getAttribute("data-search")!="text-search")) {
  ...
  if (where = checkSearch())
    send+= "&search=" + where;
  else
    send+= "&goto=" + goto;
  
  if (queryS(".tablink-active").innerHTML.toLowerCase()=="xml")  
    send+= "&tab=xml";
  
  if (action=="asset" || action=="template" || action=="templates")  
    actions.push(send);
  //log (actions);
...
/*
     FILE ARCHIVED ON 01:53:19 Aug 25, 2024 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 08:30:38 Sep 18, 2024.
...
playback timings (ms):
  ...
  PetaboxLoader3.datanode: 42.904 (4)
  PetaboxLoader3.resolve: 77.713 (2)
  load_resource: 89.476
*/